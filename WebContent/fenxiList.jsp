					...
					<div class="g_box">
						...
						<div class="g_bd">
							...
							<div class="g_articlelist">
								<ul class="clearfix">
									<c:forEach items="${newsList }" var="news" begin="0" end="8"
										varStatus="">
										<li><span class="time"><fmt:formatDate
													value="${news.addtime }" pattern="yyyy-MM-dd" /></span> <a
											href="findfenxiDetail.action?method=${news.newsid }"
											class="img" title="${news.newstitle }"> <img
												src="/upload/${news.uuid }.jpg" alt="${news.newstitle }" />
										</a>
											<h3 class="tit">
												<a href="findfenxiDetail.action?method=${news.newsid }">${news.newstitle }</a>
											</h3>
											<div
												style="height: 40px; line-height: 20px; margin-top: 3px; color: #a78966; overflow: hidden;">${news.newscontent }</div></li>
									</c:forEach>
								</ul>
							</div>
							<div align="center">
							<a  target="_self" href="findNews.action?pages=1">首页</a>&nbsp&nbsp
								<c:if test="${pages ne 1 }">
									<span><a target="_self"
										href="findNews.action?pages=${prepages }">上一页</a>&nbsp</span>
								</c:if>
								<c:if test="${nextpages le totalpages }">
									<span><a target="_self"
										href="findNews.action?pages=${nextpages }">下一页</a>&nbsp</span>
								</c:if>
								<a  target="_self" href="findNews.action?pages=${totalpages }">末页</a>&nbsp&nbsp
