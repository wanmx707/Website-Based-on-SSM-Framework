...
<body>
	<h1> 稿件&nbsp;&gt;&gt;&nbsp;管理文章稿件</h1>
	<form id="delNews" action="delNewsByid.action" method="get">
		<input type="hidden" name="newsid" id="delNewsid">
	</form>
	<form id="passNews" action="passNewsByid.action" method="get">
		<input type="hidden" name="newsid" id="passNewsid">
	</form>
	<form id="backNews" action="backNewsByid.action" method="get">
	...
	<div style="text-align: right; padding-right: 40px;">
		<td colspan="7"><a href="findNewsstatus.action?pages=1">首页</a>
			<c:if test="${pages ne 1 }">
				<a href="findNewsstatus.action?pages=${prepages }">上一页</a>
			</c:if> <c:if test="${nextpages le totalpages }">
				<a href="findNewsstatus.action?pages=${nextpages }">下一页</a>
			</c:if> <a href="findNewsstatus.action?pages=${totalpages }">末页</a>
