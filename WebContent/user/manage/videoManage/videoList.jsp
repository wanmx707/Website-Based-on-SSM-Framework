...
<body>
    ...
    <tbody>
      ...
      <tr <c:if test="${status.index mod 2 ne 0 }">bgcolor="#cccccc"</c:if>>
        ...
        <td width="10%" height="7%">
          <a href="findvideoByID.action?method=${uservideoList.videoid }">${uservideoList.videotitle }</a></td>
        <td width="40%" height="7%">${uservideoList.videocontent }</td>
        <td width="20%" height="7%">
          <fmt:formatDate value="${uservideoList.videoaddtime }"
								pattern="yyyy年MM月dd日 HH:mm:ss" />
						</td>
        <td width="10%" height="7%">${uservideoList.ststus eq 1?"审核通过":"待审核" }</td>
        <td width="10%" height="7%">
          <!-- <a href="#">更新</a> -->
          <a href="javascript:doDel(${uservideoList.videoid });">删除</a>
          <a href="findvideoByID.action?method=${uservideoList.videoid }">查看</a>
        </td>
      </tr>
      </c:forEach>
      </c:if>      
  ...
  <div style="text-align:right;padding-right:40px;">
    <td colspan="7">
      <a href="findvideoByPage.action?pages=1">首页</a>
      <c:if test="${pages ne 1 }">
      <a href="findvideoByPage.action?pages=${prepages }">上一页</a>
      </c:if>
      <c:if test="${nextpages le totalpages }">
      <a href="findvideoByPage.action?pages=${nextpages }">下一页</a>
      </c:if>
      <a href="findvideoByPage.action?pages=${totalpages }">末页</a>
